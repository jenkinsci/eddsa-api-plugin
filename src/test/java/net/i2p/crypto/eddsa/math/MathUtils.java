...
public class MathUtils {
    ...
    public static BigInteger toBigInteger(final byte[] bytes) {
        ...
        for (int i = 0; i < bytes.length; i++) {
            b = b.add(
                    BigInteger.ONE.multiply(BigInteger.valueOf(bytes[i] & 0xff)).shiftLeft(i * 8));
        }
    ...
    public static byte[] multiplyAndAddModGroupOrder(final byte[] a, final byte[] b, final byte[] c) {
        final BigInteger result =
                toBigInteger(a).multiply(toBigInteger(b)).add(toBigInteger(c)).mod(groupOrder);
    ...
    public static GroupElement toGroupElement(final byte[] bytes) {
        ...
        final BigInteger tmp = u.multiply(v.pow(7))
                .modPow(BigInteger.ONE.shiftLeft(252).subtract(new BigInteger("3")), getQ())
                .mod(getQ());
    ...
    public static GroupElement addGroupElements(final GroupElement g1, final GroupElement g2) {
        ...
        if ((g1.getRepresentation() != GroupElement.Representation.P2
                        && g1.getRepresentation() != GroupElement.Representation.P3)
                || (g2.getRepresentation() != GroupElement.Representation.P2
                        && g2.getRepresentation() != GroupElement.Representation.P3)) {
        ...
        // d = -121665/121666
        BigInteger dx1x2y1y2 =
                d.multiply(g1x).multiply(g2x).multiply(g1y).multiply(g2y).mod(getQ());
        BigInteger x3 = g1x.multiply(g2y)
                .add(g2x.multiply(g1y))
                .multiply(BigInteger.ONE.add(dx1x2y1y2).modInverse(getQ()))
                .mod(getQ());
        BigInteger y3 = g1x.multiply(g2x)
                .add(g1y.multiply(g2y))
                .multiply(BigInteger.ONE.subtract(dx1x2y1y2).modInverse(getQ()))
                .mod(getQ());
        BigInteger t3 = x3.multiply(y3).mod(getQ());

        return GroupElement.p3(
                g1.getCurve(), toFieldElement(x3), toFieldElement(y3), getField().ONE, toFieldElement(t3));
    }
    ...
    public static GroupElement doubleScalarMultiplyGroupElements(
            final GroupElement g1, final FieldElement f1, final GroupElement g2, final FieldElement f2) {
    ...
    public static GroupElement negateGroupElement(final GroupElement g) {
        ...
        return GroupElement.p3(
                g.getCurve(), g.getX().negate(), g.getY(), g.getZ(), g.getT().negate());
    }
